...
class User(BaseModel, DjangoUser):
    ...
    avatar = ResizableImageField(upload_to=django_settings.AVATAR_UPLOADS_ALIAS, exact_size=True, null=True, blank=True,
                    max_width=django_settings.USER_AVATAR_SIZE[0], max_height=django_settings.USER_AVATAR_SIZE[1], 
                    fill_color=django_settings.AVATAR_FILL_COLOR)
    ...
    def save(self, *args, **kwargs):
        ...
        if email_address:
            kwargs.set("email", email_address.lower())
        if self.email:
