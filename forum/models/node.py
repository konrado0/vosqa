...
_node_render_context = {"MEDIA_URL": djsettings.MEDIA_URL}
...
class NodeContent(models.Model):
    ...
    main_image = models.ForeignKey('Image', null=True, blank=True, default = None)    
    ...
    def html(self):
        try:
            return mark_safe(self.body % _node_render_context)
        except:
            logging.exception("Error while rendering body:\n%s" % self.body)
            return mark_safe(u"""<p class="error">We are sorry, but error happened while rendering this content :(<br />
                                 Report has been filed and our team will handle it as soon as possible.</p>""")     
    ...
    def top_tags(self):        
        tags = self.tagname_list()
        size = len(tags[0])
        result = [tags[0], ]
        for tag in tags[1:]:
            if len(tag) + size <= settings.FORM_MAX_NUMBER_OF_CHARACTERS_IN_TOP_TAGS and len(result) < 3: 
                result.append(tag)
                size += len(tag)
        return result
...
class Node(BaseModel, NodeContent):
    ...
    def activate_revision(self, user, revision):
        ...
        self.main_image = revision.main_image
...
#         dirty = self.get_dirty_fields()
        ...
        if self._original_state["tagnames"] == self.__dict__['tagnames']:
            return None
        else:
    ...
    def mark_deleted(self, action):
        ...
        if action:
            ...
            tagz = Tag.objects.filter(name__in=self.tagname_list())
            if tagz.exists():
    ...
    def save(self, *args, **kwargs):
        if not self.id:
            ...
            self.active_revision = self._create_revision(self.author, 1, title=self.title, tagnames=self.tagnames,
                                                         body=self.body, main_image=self.main_image)
