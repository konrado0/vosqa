...
def gravatar(user, size):
    #if user has local avatar, use it
    if user.avatar:
        return mark_safe('<img class="gravatar" width="%s" height="%s" src="%s" alt="%s\'s avatar" />' % (
                size, size, user.avatar.url, template.defaultfilters.urlencode(user.username)))    
...
# Usage: {% show_badge badge show_count? %}
@register.simple_tag
def show_badge(badge, count=0):
    count_html = u''
    if count:
        count_html = u'<strong class="medal"> &#215; %s</strong>' % intcomma(count)

    #<span class="badge%s">&#9679;</span>&nbsp;
    html_output = u"""
    <div class="icon badge%s"><div class="shine">
    <a href="%s" title="%s" class="medal">%s</a>
    </div></div>%s
    """ % (badge.type, badge.get_absolute_url(), badge.description, badge.name, count_html)
...
def diff_date(date, limen=2):
    ...
    if date.year != now.year:
        return dateformat.format(date, 'd M Y') #'d M \'y, H:i')
    elif days > 2:
        return dateformat.format(date, 'd M') #'d M, H:i')
        ...
        for line in source.splitlines():
            ...
            if m:
                ...
                try:
                    ...
                    logging.exception("Error in declare tag, when evaluating: %s" % m.group(3).strip())
        return ''
