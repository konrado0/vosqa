...
class PaginatorContext(object):
    ...
    def list_template(self, request, session_prefs):
        list_template=request.GET.get('list_template', None)        
        
        if not session_prefs:
            session_prefs = self.preferences(request)
            
        if list_template:
            if list_template not in  djsettings.ALLOWED_LIST_TEMPLATES:
                logger.warning('Unrecognized list_template parameter received: %s' % list_template)
                raise Http404
            session_prefs[self.TEMPLATE] = list_template
        else:
            list_template = session_prefs.get(self.TEMPLATE, djsettings.DEFAULT_LIST_TEMPLATE)
    ...
    @property
    def TEMPLATE(self):
        return self.prefix and "%s_%s" % (self.prefix, _('list_template')) or _('list_template')
