...
# Raw urls for endless-scrolling
raw_urls = patterns('',
    url(r'^%s$' % _('questions/'), app.readers.raw_questions, name='raw_questions'),
    url(r'^%s%s$' % (_('questions/'), _('unanswered/')), app.readers.raw_unanswered, name='raw_unanswered'),
    url(r'^%s(?P<mode>[\w\-]+)/(?P<user>\d+)/(?P<slug>.*)/$' % _('questions/'), app.readers.raw_user_questions, name='raw_user_questions'),
    url(r'^%s(?P<tag>.*)/$' % _('tags/'), app.readers.raw_tag, name='raw_tag_questions'),
    url(r'^%s$' % _('search/'), app.readers.search, name='raw_search'),
)

core_urls = (
    url(r'^$', app.readers.index, name='index'),    
    url(r'^raw/', include(raw_urls)),    
    ...
    url(r'^%s(?P<path>.*)$' % _(djsettings.MEDIA_URL[1:]), 'django.views.static.serve', {
                    'document_root': djsettings.MEDIA_ROOT }, name='uploaded_file',),
    
    url(r'^%s$' % _('faq/'), app.meta.from_template, {'name': 'faq'}, name='faq'),
    url(r'^%s$' % _('about/'), app.meta.from_template, {'name': 'about'}, name='about'),
    url(r'^opensearch\.xml$', app.meta.opensearch, name='opensearch'),
    url(r'^%s$' % _('privacy/'), app.meta.from_template, {'name': 'privacy'}, name='privacy'),
    url(r'^%s$' % _('terms/'), app.meta.from_template, {'name': 'terms'}, name='terms'),
    url(r'^%s$' % _('logout/'), app.auth.signout, name='logout'), #app.meta.logout
    ...
    url(r'^%s(?P<id>\d+)/' % _('node_content/'), app.commands.node_content, name='node_content'),
    ...
    url(r'^%s(?P<id>\d+)/(?P<slug>.*)/%s$' % (_('users/'), _('badges/')), app.users.user_badges, name='user_badges'),
...
#     url(r'^%s$' % _('upload/'), app.writers.upload, name='upload'),
    ...
    url(r'^flow/$', app.readers.flow, name='flow'),
)

if djsettings.DEBUG:
    from django.conf.urls.static import static
    core_urls+= tuple(static(djsettings.STATIC_URL, document_root=djsettings.STATIC_ROOT))
else:
    core_urls+= (url(r'^%s(?P<path>.*)$' % _(djsettings.STATIC_URL[1:]), 'django.views.static.serve', {
                    'document_root': djsettings.STATIC_ROOT }, name='static_file'), )
