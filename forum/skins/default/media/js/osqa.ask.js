...
//        }
        ...
        $.post(related_questions_url, {title: q}, function(data) {
            if (data) {
                ...
                if(data.length == 0) {
                	relatedQuestionsDiv.animate({'height':'70'}, 350);
                	relatedQuestionsDiv.html('<br /><div align="center">No questions like this have been found.</div>');
                    return;
                }

                relatedQuestionsDiv.animate({'height':'170'}, 350);
                var html = '<h3>Related questions:</h3>';
    ...
    var timer = null;
    function suggestions_on_timout() {
    	if(timer) window.clearTimeout(timer);    	 
    	timer = window.setTimeout(reload_suggestions_box, 1000);
    }

    $input.keyup(suggestions_on_timout);
