...
function tagsStartWithHash(tags) {
	return tags.replace(/#/g, '').replace(/\w\w+/g, function(match) {		
		return '#'+match;
	});
}
...
var response_commands = {
    ...
    unmark_deleted: function(post_type, post_id) {
        if (post_type == 'answer') {
            ...
            $answer.find("a").filter(function () {
                return this.innerHTML == 'undelete';
            }).text('delete');
        } else {
            var $container = $('#question-table');
...
function show_dialog (extern) {
    ...
    if (options.dim === false) {
        ...
        // +1 is a fix for chrome display
        options.dim = {w: $dialog.width()+1, h: $dialog.height()};
    ...
    $('div.comment-form-container').each(function() {
        ...
        if ($form.length) {
            ...
            $button.click(function(evt) {
                ...
                var text = $("<div>"+$textarea.val()+"</div>").text();
                if(text.length < $textarea.val().length) {
                	show_message(evt, "HTML tags are not allowed in comments.", function() {$textarea.val(text);});
                	return false;
                }                
...
function pickedTags(){
    ...
    var handlePickedTag = function(obj, evt, reason){
    	var input = $(obj).prev();
    	var tagname = $.trim(input.val()).replace(/#/g, '');
        ...
        if (!(tagname in to_target)){
            ...
            sendAjax(tagname,reason,'add',function(data){
            	if(!data) {
            		show_message(evt, "'"+ tagname+"' is not a valid tag");
            		return;
            	}
            	input.val("");
    ...
    return {
        init: function(){
            ...
            $("#interestingTagAdd").click(function(evt){handlePickedTag(this, evt, 'good');});
            $("#ignoredTagAdd").click(function(evt){handlePickedTag(this, evt, 'bad');});
        }
    };
}
