...
def GetListTemplate(request):
    template = request.GET.get('list_template', None)
    if template and template in djsettings.ALLOWED_LIST_TEMPLATES:
        return template
    logger.warning('Raw view called with unrecognized list_template parameter: %s' % template)
    raise Http404
...
flow_pagesizes = (djsettings.DEFAULT_FLOW_PAGESIZE, )
class QuestionListPaginatorContext(pagination.PaginatorContext):
    def __init__(self, id='QUESTIONS_LIST', prefix='', pagesizes=flow_pagesizes, default_pagesize=djsettings.DEFAULT_FLOW_PAGESIZE):
...
@decorators.render('flow.html')
def index(request):
    paginator_context = QuestionListPaginatorContext(pagesizes=flow_pagesizes, default_pagesize=djsettings.DEFAULT_FLOW_PAGESIZE)
...
@decorators.render('index.html')
def flow(request):
    paginator_context = QuestionListPaginatorContext(pagesizes=djsettings.DEFAULT_PAGESIZES, default_pagesize=djsettings.DEFAULT_PAGESIZE)
    paginator_context.base_path = reverse('questions')
    return question_list(request,
                         Question.objects.all(),
                         base_path=reverse('questions'),
...
def _unanswered(request):
    return question_list(request,
                         Question.objects.all().exclude(id__in=Question.objects.filter(children__marked=True).distinct()).exclude(marked=True),
                         _('open questions without accepted answer'),
                         None,
                         _("Unanswered"))

# @decorators.render('questions.html', 'unanswered', _('unanswered'), weight=400)
@decorators.render('flow.html', 'unanswered', _('unanswered'), weight=400)
def unanswered(request):
    return _unanswered(request)
    
def raw_unanswered(request):
    return render_to_response(GetListTemplate(request), _unanswered(request))


def _questions(request):
    return question_list(request,
                         Question.objects.all(),
                         _('questions'))

# @decorators.render('questions.html', 'questions', _('questions'), weight=0)
@decorators.render('flow.html', 'questions', _('questions'), weight=0)
def questions(request):
    return _questions(request)

def raw_questions(request):
    return render_to_response(GetListTemplate(request), _questions(request))


def _tag(request, tag):
    try:
    ...
    questions = Question.objects.all().filter(tags__id=tag.id)
    ...
    if request.method == "GET":
        ...
        if user is not None:
            try:
                questions = questions.all().filter(author=User.objects.get(username=user))
    ...
    question_context = question_list(request,
                         questions,
                         mark_safe(_(u'questions tagged <span class="tag" style="font-size: 14px">%(tag)s</span>') % {'tag': tag}),
                         None,
                         mark_safe(_(u'Tagged')),
                         False)
...
@decorators.render('flow.html')
def tag(request, tag):
    return _tag(request, urlunquote(tag))

def raw_tag(request, tag):
    return render_to_response(GetListTemplate(request), _tag(request, tag))



def _user_questions(request, mode, user, slug):
    ...
    if mode == _('asked-by'):
        ...
        questions = Question.objects.all().filter(children__author=user, children__node_type='answer').distinct()
...
@decorators.render('flow.html', 'questions', tabbed=False)
def user_questions(request, mode, user, slug):
    return _user_questions(request, mode, user, slug)

def raw_user_questions(request, mode, user, slug):
    return render_to_response(GetListTemplate(request), _user_questions(request, mode, user, slug))
...
def question_list(request, initial,
    ...
    context = {
        'questions' : questions.distinct().prefetch_related('author', 'main_image'),
...
# @decorators.render('questions.html')
def _question_search(request, keywords):
    rank_feed = False
    ...
    return question_list(request, initial,
                         mark_safe(_("questions matching:<br /><b>'%(keywords)s'</b>") % {'keywords': keywords}),
                         None,
                         _("Search results"),
                         paginator_context=paginator_context,
...
def question_search(request, keywords):
    return _question_search(request, keywords)
    
def raw_question_search(request):
    keywords = request.GET.get("q")
    return render_to_response(GetListTemplate(request), _question_search(request, keywords))
...
def question(request, id, slug='', answer=None):
    ...
    if request.POST:
        answer_form = request.session.get('answer_form_with_errors', None)
        if not answer_form:
            answer_form = AnswerForm(request.POST, user=request.user)
        else:
            del request.session['answer_form_with_errors']
    else:
    ...
    "question" : question,
    "answer" : answer_form,
    "answers" : answers.prefetch_related('author', 'parent'),
