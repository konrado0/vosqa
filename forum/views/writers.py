...
#         instance.save()
# 
# #         storage = FileSystemStorage(UPFILES_FOLDER, UPFILES_ALIAS)
...
# 
#         result = xml_template % ('Good', '', instance.image.url)
...
def _get_image(form):
    url = form.fields["image"].get_url()
    image = None
    if url:
        src = url
        if url.startswith(djsettings.APP_URL):
            src = src.replace(djsettings.APP_URL, u"/", 1)
        if src.startswith(djsettings.MEDIA_URL):
            try:            
                image = Image.objects.get(image=src.replace(djsettings.MEDIA_URL,u""))                
            except:
                logger.error(u'Unable to locate img stored under url %s in Image table' % url)
            
    if not image:
        image = Image(image=form.cleaned_data["image"], upload_url=form.fields["image"].get_url())
        image.save()
...
def _edit_question(request, question, template='question_edit.html', summary='', action_class=ReviseAction,
    ...
    if request.method == 'POST':
        ...
        if 'select_revision' in request.POST:
            ...
            form = EditQuestionForm(question, request.user, revision, data=request.POST, files=request.FILES)
...
def answer(request, id):
    ...
    if request.POST:
        ...
        if request.session.pop('reviewing_pending_data', False) or not form.is_valid():
            ...
            request.session['answer_form_with_errors'] = form
