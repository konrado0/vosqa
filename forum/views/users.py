...
@decorators.render('users/users.html', 'users', _('people'), weight=200)
def users(request):
...
def edit_user(request, id, slug):
    ...
    if request.method == "POST":
        form = EditUserForm(user, request.POST, files=request.FILES)
        if form.is_valid():
            ...
            if form.cleaned_data['avatar']:
                #TODO: Figure out why this ugly trick is needed
                f = form.cleaned_data['avatar']
                user.avatar.save(f.name, f)
                user._original_state['avatar'] = None
...
def user_profile(request, user, **kwargs):
    ...
    badges = [(Badge.objects.get(id=b['id']), b['count'] if b['count'] != 1 else 0) for b in
...
_all_badges = sorted([Badge.objects.get(id=id) for id in BadgesMeta.by_id.keys()], lambda b1, b2: cmp(b1.name, b2.name))

@user_view('users/badges.html', 'badges', _('badges'), _('user badges'))
def user_badges(request, user, **kwargs):
    badges = [(Badge.objects.get(id=b['id']), b['count'] if b['count'] != 1 else 0) for b in
              Badge.objects.filter(awards__user=user).values('id').annotate(count=Count('cls')).order_by('-count')]        
    
    return {"view_user": user, "badges": badges}
