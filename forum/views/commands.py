...
def comment(request, id):
    ...
    if len(comment_text) != comment_length:
        raise CommandException(_("HTML tags are not allowed in comments."))

    if len(comment_text) < settings.FORM_MIN_COMMENT_BODY:
        raise CommandException(_("At least %d characters required on comment body (HTML tags are not allowed).") % settings.FORM_MIN_COMMENT_BODY)
    ...
    if comment.active_revision.revision == 1:
        ...
        'insert_comment': [
                ...
                reverse('node_content', kwargs={'id': comment.id}),
...
def node_content(request, id):
    ...
    node = get_object_or_404(Node, id=id)
    return HttpResponse(strip_tags(node.active_revision.body), content_type="text/plain")
...
def matching_tags(request):
    ...
    possible_tags = Tag.active.filter(name__icontains = request.GET['q'].replace(u"#", u""))
    tag_output = ''
