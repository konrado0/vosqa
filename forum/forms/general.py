...
_password = re.compile('[A-Za-z0-9@#$%%^&+=\!\?]{%d,}' % settings.MIN_PASSWORD_LENGTH)

class SetPasswordForm(forms.Form):
    password1 = forms.CharField(widget=forms.PasswordInput(attrs=login_form_widget_attrs),
                                label=_('choose password'), min_length=settings.MIN_PASSWORD_LENGTH,
                                error_messages={'required':_('password is required'), 
                                    'min_length':_('Password needs to be at least %d characters long.' % settings.MIN_PASSWORD_LENGTH), 
                                    'accepted_char':_('Accepted characters are A-Z, digits and any of "@#$\%^&+=!?".')},
                                )
    ...
    def clean_password1(self):
        """
        Validates that only accepted characters are used.
        
        """
        if self.cleaned_data['password1']:
            if not _password.match(self.cleaned_data['password1']): 
                raise forms.ValidationError(self.fields['password1'].error_messages['accepted_char'])
